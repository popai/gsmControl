...
//const char ATE0[] PROGMEM = "ATE0"; 				//< Echo off
...
int DEBUG_LED = 13;                // LED connected to digital pin 25
...
GSM::GSM(void)
{
	...
	comm_line_status = STATUS_NONE;
	// not registered yet
	module_status = STATUS_NONE;

	// initialization of speaker volume
	//last_speaker_volume = 0;
	p_comm_buf = NULL;              // pointer to the communication buffer
	comm_buf_len = 0;              // num. of characters in the buffer
	rx_state = STATUS_NONE;                // internal state of rx state machine
	start_reception_tmout = 0; // max tmout for starting reception
	interchar_tmout = 0;       // previous time in msec.
	prev_time = 0;        // previous time in msec.
...
char GSM::InitSMSMemory(void)
{
	...
	if (AT_RESP_OK
			== SendATCmdWaitResp("AT+CPMS=\"SM\",\"SM\",\"SM\"", 1000, 1000,
					"+CPMS:", 10))
	{
		ret_val = 1;
	}
 ...
 **********************************************************/
char GSM::GetAuthorizedSMS(byte position, char *phone_number, char *SMS_text,
		byte max_SMS_len, byte first_authorized_pos, byte last_authorized_pos)
{
	byte ret_val = -1;
	byte i;
